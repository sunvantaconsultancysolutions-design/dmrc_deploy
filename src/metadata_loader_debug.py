...
def build_chunk_records(parsed_json: dict, source_file: str, carry_over_record: Optional[ChunkRecord] = None):
    ...
    # ==== TEMPORARY DEBUG COUNTERS (no logic impact, remove after diagnosis) ====
    _dbg_total_clause_entries = 0   # entries whose chunk_type classifies as "clause"
    _dbg_clause_records = 0         # of those, how many became actual records
    _dbg_cover_pages = 0
    _dbg_indexes = 0
    _dbg_marginalia = 0
    _dbg_continuations_merged = 0
    _dbg_empty_skipped = 0
    _dbg_stamps_attached = 0
    print(f"\n########## START FILE: {source_file} ##########")
    # ==== END DEBUG COUNTER INIT ====
    ...
    for page in parsed_json.get("pages", []):
        ...
        for clause in page.get("clauses", []):
            ...
            # ==== TEMPORARY DEBUG: print every clauses[] entry processed ====
            print(
                "---------------------------------\n"
                f"pdf_page={pdf_page}\n"
                f"heading={heading!r}\n"
                f"clause_no={clause_no!r}\n"
                f"continues_previous={continues_previous}\n"
                f"chunk_type={chunk_type}\n"
                f"text_length={len(text)}"
            )
            # ==== END DEBUG ENTRY PRINT ====
            ...
            if chunk_type == "stamp":
                ...
                if last_record is not None:
                    ...
                    # ==== TEMPORARY DEBUG ====
                    _dbg_stamps_attached += 1
                    print(
                        "STAMP ATTACHED:\n"
                        f"  page={pdf_page}\n"
                        f"  attached_to_chunk={last_record.metadata.get('chunk_id')}\n"
                    ...
                    print(
                        "STAMP ATTACHED:\n"
                        f"  page={pdf_page}\n"
                        f"  attached_to_chunk=<pending, no last_record yet>\n"
                        f"  organization={organization}"
                    )
                    # ==== END DEBUG ====
                continue
            ...
            if chunk_type == "continuation":
                if open_record is not None:
                    _dbg_before_len = len(open_record.text)
                    open_record.text = (open_record.text.rstrip() + "\n\n" + text.strip()).strip()
                    if low_confidence:
                        open_record.metadata["low_confidence"] = True
                    _recompute_hash(open_record)
                    # ==== TEMPORARY DEBUG ====
                    _dbg_continuations_merged += 1
                    print(
                        "MERGED CONTINUATION:\n"
                        f"  page={pdf_page}\n"
                        f"  merged_into_clause={open_record.metadata.get('clause_no')}\n"
                        f"  added_chars={len(open_record.text) - _dbg_before_len}"
                    )
                    # ==== END DEBUG ====
                else:
                    # ==== TEMPORARY DEBUG ====
                    print(
                        f"NO OPEN RECORD TO MERGE INTO (page={pdf_page}) -- "
                        f"falling back to standalone clause record"
                    )
                    # ==== END DEBUG ====
            ...
            if chunk_type == "clause" and not text.strip():
                # ==== TEMPORARY DEBUG ====
                _dbg_total_clause_entries += 1
                _dbg_empty_skipped += 1
                print(
                    "SKIPPED EMPTY CLAUSE:\n"
                    f"  page={pdf_page}\n"
                    f"  clause={clause_no}\n"
                    f"  heading={heading}"
                )
            ...
            if chunk_type == "clause":
                _dbg_total_clause_entries += 1
                _dbg_clause_records += 1
            elif chunk_type == "cover_page":
                _dbg_cover_pages += 1
                ...
                _dbg_marginalia += 1
            print(
                f"NEW RECORD CREATED: chunk_type={chunk_type} "
                f"chunk_id={chunk_id} clause_no={clause_no!r}"
            )
            # ==== END DEBUG ====
    ...
    # ==== TEMPORARY DEBUG SUMMARY ====
    print(
        "==========================\n"
        "SUMMARY\n"
        "==========================\n"
        f"Source file: {source_file}\n"
        f"Total clause entries in JSON: {_dbg_total_clause_entries}\n"
        f"Total records created: {len(records)}\n"
        f"Clause records: {_dbg_clause_records}\n"
        f"Cover pages: {_dbg_cover_pages}\n"
        f"Indexes: {_dbg_indexes}\n"
        f"Marginalia: {_dbg_marginalia}\n"
        f"Continuations merged: {_dbg_continuations_merged}\n"
        f"Empty clauses skipped: {_dbg_empty_skipped}\n"
        f"Stamps attached: {_dbg_stamps_attached}\n"
        f"########## END FILE: {source_file} ##########\n"
    )
    # ==== END DEBUG SUMMARY ====
