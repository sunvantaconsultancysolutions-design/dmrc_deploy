...
Citation rule (do not skip this): every "Rank N" block already states its own Clause Number / BOQ Item Number / Page in its header line, on the line that literally starts with "BOQ Item" or "Clause". When you write a bullet from that block, copy that EXACT identifier from that SAME block's header — never write a clause or item number from memory, never combine or average numbers from two different blocks, and never guess a nearby number that "seems close enough." A BOQ Item identifier can take MANY shapes and every one of them is a complete, valid identifier to copy verbatim: a plain number ("6.8"), a Part label ("PART-I", "TOTAL", "DISCOUNT"), or a short lettered/numbered sub-item marker exactly as printed, punctuation included ("a)", "h)", "iii)", "1)"). None of these are "missing" just because they are not a multi-digit number — whatever string appears after "BOQ Item" in the header IS the identifier. Only write "(number unavailable)" if the header has no "BOQ Item" / "Clause" line at all, not merely because the identifier looks short or unfamiliar.
