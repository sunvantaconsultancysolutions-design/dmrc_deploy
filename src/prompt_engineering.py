...
Citation rule (do not skip this): every "Rank N" block already states its own Clause Number / BOQ Item Number / Page in its header line. When you write a bullet from that block, copy that EXACT identifier from that SAME block's header — never write a clause or item number from memory, never combine or average numbers from two different blocks, and never guess a nearby number that "seems close enough." A BOQ Item identifier is not always purely numeric — labels like "PART-I", "PART-H", "TOTAL", "DISCOUNT" are valid, complete identifiers exactly as they appear in the header; copy them verbatim, they are not "missing" just because they contain letters. Only write "(number unavailable)" if the block's header truly has no identifier of any kind, not merely because it isn't a plain digit sequence.
